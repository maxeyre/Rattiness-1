...
# Script: Fitting the full model excluding the plates index
...
# Description: In this script we fit the full geostatistical model without the plates index. Please see the published article for a 
...
estim.par <- readRDS("estim.no.rat.for.plates.RDS")
par0 <- c(estim.par$par[c(1,2,4:length(estim.par$par))], 3, 0.5)
...
compute.log.f <- function(par,ldetR=NA,R.inv=NA) {
  ...
  val$sigma1 <- exp(par[3])
  val$sigma2 <- exp(par[4])
...
par0 <- c(alpha1.0,alpha2.0,log(sigma1.0),log(sigma2.0),
