...
# Script: Fitting the full model excluding the signs index
...
# Description: In this script we fit the full geostatistical model without the signs index. Please see the published article for a 
...
estim.par <- readRDS("estim.no.rat.for.signs.RDS")
par0 <- c(estim.par$par[2:length(estim.par$par)], 3, 0.5)
...
compute.log.f <- function(par,ldetR=NA,R.inv=NA) {
  ...
  val$sigma2 <- exp(par[3])
  val$sigma3 <- exp(par[4])
...
par0 <- c(alpha2.0,alpha3.0,log(sigma2.0),log(sigma3.0),
...
saveRDS(estim.par, "par_exc_signs.RDS")
