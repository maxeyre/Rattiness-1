# PredictionExcSigns.R

# Script: Spatial prediction of Rattiness for the full model excluding the signs index
...
# the dataset 2-PdLPredictionGrid.csv using the full geostatistical model without the signs index. Please see the 
# published article for a detailed explanation of the methodology.
...
estim.par <- readRDS("par_exc_signs.RDS")
alpha2.0 <- estim.par$par[1]
alpha3.0 <- estim.par$par[2]
beta0 <- estim.par$par[5:(p+4)]
sigma2.0 <- exp(estim.par$par[3])
sigma3.0 <- exp(estim.par$par[4])
phi0 <- exp(estim.par$par[p+5])
